...
def prompt_axes(xlogbase=None, ylogbase=None):
    ...
    while not finished:
        ...
        point_value = [float(raw_input('What is the x component?: ')), float(raw_input('What is the y component?: '))]
        if xlogbase:
            point_value[0] = log(point_value[0], xlogbase)
        if ylogbase:
            point_value[1] = log(point_value[1], ylogbase)
        point_map[raw_point] = tuple(point_value)
        finished =  bool(raw_input('Add another point? (y)/n: '))
    return point_map
...
def generate_transform(axis_point_map):
    ...
    for ii,real_coord,raw_coord in zip(xrange(num_pairs), real_coords, raw_coords):
